...
-(void)drawRect:(CGRect)rect {
    ...
    self.decreaseButton.titleLabel.font = [UIFont systemFontOfSize: 30];
    self.increaseButton.titleLabel.font = [UIFont systemFontOfSize: 30];
...
-(void)setValue:(CGFloat)value {
    ...
    if (_value != value) {
        if (!(value < self.minimumValue || value > self.maximumValue)) {
            NSString* valueString = [NSString stringWithFormat:@"%0.*f GB", (int)self.fractionDigits, value];
            if ((self.fractionDigits > 0) && ([valueString doubleValue] == [valueString integerValue])) {
                valueString = [NSString stringWithFormat:@"%ld GB", (long)[valueString integerValue]];
